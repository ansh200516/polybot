...
2. **DONE (2026-07-13) — resolution backfill in `btc5m_report.py`** (offline, Python-only, no Rust change; tested live against a real resolved window): reconstructs each window's slug from the shadow row's `ts_ms`+`secs_to_go`, fetches `eventMetadata.{priceToBeat,finalPrice}` (verifying `conditionId`), caches to a `btc5m_resolution` table, then reports (A) proxy-strike gate, (B) **TRUE-strike gate with realized win% vs the real outcome**, (C) proxy↔true **basis** distribution, (D) model **calibration** (Brier). Closes follow-ups #2/#3 — the gate is now trustworthy without touching the live loop.
