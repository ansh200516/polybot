...
2. **Realized-PnL fee accuracy — ✅ DONE (2026-07-14, commit b83066c).** Verified the live path does NOT capture the on-chain fee (`poll_fills` models it from the order's `Bps(0)` → `fee=0`). btc5m now folds the `crypto_fees_v2` fee (`0.07·p·(1−p)`, per fill, via `crypto_taker_fee_micro`) into the recorded position `cost_micro` at entry, so `day_realized("btc5m")` and the `pnl`/report are **net-of-fee** and Gate-2 matches the fee-aware Gate-1. (Shared `live.rs` untouched; entry discipline already modeled the fee, so this was measurement-only.)
