...
// The validated pure ranking + freshness primitives now live in the shared
// `pm_ingestion::smart_money` module (ONE source of truth for this backtest AND
...
// ---------------------------------------------------------------------------
// 1. Ranking — the spectrum under test (now in `pm_ingestion::smart_money`)
// ---------------------------------------------------------------------------
//
// The `Ranking` enum and the OUT-OF-SAMPLE selection (`TraderRecord`,
// `trader_records`, `rank_wallets_oos`) moved to `pm_ingestion::smart_money` and
// are re-exported above. The legacy closed-positions ranking below
// (`rank_wallets`) is kept here with its unit tests.
...
pub fn simulate_signal(
    ...
    // Delegated to the shared `within_drift` primitive so the backtest and the
    // live copy strategy apply the SAME rule; a degenerate `trigger_price == 0`
    // is NOT within drift, so it skips — identical to the old `+inf > max_drift`.
    if let Some(max_drift) = p.max_drift
        && !within_drift(entry_px, sig.trigger_price, max_drift)
    {
        return SimResult::Skipped;
    }
